<table width=180 align=right bgcolor=#AAAAAA>
<tr>
<td width=180 valign=top>

**TOC**

</td>
</tr>
</table>

This is a historical summary of the [Patches](Patch.md) of
PlanetSide.

To find a complete listing by revision number, view the Patches
category, linked at bottom.

[Unnumbered Patches](Unnumbered_Patches.md)

## 2003

### May

- [1.0.0](1.md.0.0) - [PlanetSide](PlanetSide.md)
  Launches on May 20th!
- [1.1](1.md.1) - [Jamming](Jammer_grenade.md) of
  friendly [Combat Engineering](Combat_Engineering.md)
  disabled. [Implant](Implant.md) confirmation removed.
  [Tells](Tell.md) no longer case sensitive.
- [1.2](1.md.2) - [Zone](Zone.md) crash fixes, weather
  tweaks, and [Server](Server.md) listing tweaks added. Base
  kill [Experience](Experience.md) doubled from 50 to 100.
- [1.3](1.md.3) - Zone and game crash fixes

### June

- [1.4](1.md.4) - Vehicle Queues, Population Lock indicators,
  and Bandwidth settings. Other minor tweaks.
- [1.5](1.md.5) - [Skyguard](Skyguard.md),
  [Liberator](Liberator.md)
  _[Certification](Certification.md)_, and [Orbital
  Strikes](Orbital_Strike.md)/[EMPs](EMP.md) go live!
  Respawn into hacked bases available. [Hotspot](Hotspot.md)
  improvements.

### July

- [Liberator
  Patch](Unnumbered_Patches.md#Liberator_.287-3-03.29) -
  [Liberator](Liberator.md) goes live and is available for
  purchase! Door bug fixes.

### August

- [LLU
  Patch](Unnumbered_Patches.md#LLU_Goes_Live_-_August_14.2C_2003) -
  LLU capture system goes live! [Lasher](Lasher.md) and
  [Starfire](Starfire.md) damage degredation tweaks,
  [Basilisk](Basilisk.md) and [Wraith](Wraith.md)
  combined into single [ATV](ATV.md) cert, and HART waypoint
  exploit fixes added.
- [1.8.8](1.md.8.8) - LLU changes, ATV down to 2 cert points,
  Starfire MAX projectile matches Sparrow MAX speed
- [1.9.1](1.md.9.1) - Flashing LLU link, faster auto-drive off
  vpad, respawning no longer costs NTU
- [1.9.2](1.md.9.2) - Speedhacking detection hotfix

### September

- [1.10.10](1.md.10.10) - Bug fixes, lockers/implants/cert
  terminals hackable, equipment favorites names extended

### October

- [1.11.14](1.md.11.14) - Balance Pass and Platoons go live!
- [2.0.47](2.md.0.47) - Lattice link map hotfix

### November

- [2.0.49](2.md.0.49) - AMS zone crash fix, drop pod client
  crash fix, extra health exploit fix
- [2.1.5](2.md.1.5) - Client startup crash fix
- [2.1.6](2.md.1.6) - Ancient Tech weapons and vehicle
  tweaks/fixes. Galaxies banished from caverns, VS/TR MAX tweaks, and
  ACE tweaks.
- [2.2.13](2.md.2.13) - Keymapping tweaks, hack time and LLU
  lifespan available on map, geowarp tweaks, and module tweaks
  (including ability to spawn all types).
- [2.2.15](2.md.2.15) - Server crash fixes, animation culling
  and backpack cleanup code tweaks.

### December

- [2.3.6](2.md.3.6) - Battle Concentration Initiative (server
  merge) and Lodestar go live! HART timer changed to 5 minutes.
- [2.3.9](2.md.3.9) - Lodestar tweaks, 12 hour empire timer,
  memory and muzzle flash fixes.
- [2.3.13](2.md.3.13) - Spawning in the AMS bubble instead of
  tube, zone/client crash fixes, cavern/geowarp fixes
- [2.3.14](2.md.3.14) - Medic zone crash and striker client
  crash fixes
- [2.3.18](2.md.3.18) - Server crash fixes

## 2004

### January

- [2.3.23](2.md.3.23) - [Holiday
  Decorations](Holiday_Decorations.md) removed

### February

- [2.4.7](2.md.4.7) - Rexo armor increased by 50, empire
  poplocks tweaked for even spread three ways, multiple bug fixes
- [2.4.12](2.md.4.12) - Player response updates in testing,
  animation culling saving fix, VR terminal grief removed

### March

- [2.5.5](2.md.5.5) - [Capitols](Capitol.md), Ancient
  Tech Ammo, and cavern lock benefits go live! DC MAX CoF reduction
  and grenades turned into 3-packs.

### April

- [2.6.5](2.md.6.5) - High-res textures, auto-reload, armor
  status bar under player names, vehicle hover brakes and gear-ratio
  tweaks, infiltrator fixes, silent run changed to [Sensor
  Shield](Sensor_Shield.md).

### May

- [2.7.6](2.md.7.6) - [Fury](Fury.md),
  [Deliverer](Deliverer.md) Variants, and new
  [AMS](AMS.md) design go live! [Tower](Tower.md)
  [SOIs](SOI.md) visible at all map zoom levels, and session
  stats (K/D, [BEP](BEP.md)/[CEP](CEP.md), time)
  available

### June

- [2.7.8](2.md.7.8) - [Thunderer](Thunderer.md) and
  [Aurora](Aurora.md) tweaks, density warnings changed to
  yellow/orange/red

### July

- [2.8.5](2.md.8.5) - Merit tracking test drive & [Annual
  Terms of Service Merit](Term_of_Service.md) go live! Overlay
  grid, multiple macro banks, proximity squad invite, and other
  interface tweaks implemented.
- [2.8.7](2.md.8.7) - AGP performance and drop pod fix

### August

- [2.9.5](2.md.9.5) - [Empire
  Incentives](Empire_Incentives.md) modifications,
  [merit](Merit_Commendation.md) shoulderpad, Adaptive LOD
  added, and different [animation
  culling](Animation_Culling.md) levels added.
  [LLU](LLU.md) [sub-capitols](Sub-Capitol.md) linked
  only by [Capitol](Capitol.md) spawn LLUs again.
  [Striker](Striker.md), [Radar](Radar.md),
  [Outfit](Outfit.md) invitation and "turn left" keymapping
  fix.
- [2.9.8](2.md.9.8) - [MA](Medium_Assault.md) damage
  degredation extended. [Starfire](Starfire.md) and
  [Sparrow](Sparrow.md) range increased.
  [Zoning](Zone.md) [client](client.md) crash fix.
- [2.10.7](2.md.10.7) - [The Bending](The_Bending.md).
  [Battle Islands](Battle_Islands.md) go live!

### September

- [2.10.14](2.md.10.14) - Standard Assault Pass. Cavern
  capture experience enabled. Rearm pads/silos always rearm. Jacked
  AMS stays deployed.

### October

- [3.0.17](3.md.0.17) - [BFR](BattleFrame_Robotics.md)
  assets and hangars added. Armored Assault cert tree implemendted.
  Buggy and tank speeds increased. Tech plant requirement removed for
  [Router](Router.md) and [Sunderer](Sunderer.md)
- [3.0.18](3.md.0.18) - [Beamer](Beamer.md),
  [MAX](MAX.md) armor, avatar invisibility, and zone crash
  fix.
- [3.0.19](3.md.0.19) - Autodrive grief removed.
  [Galaxy](Galaxy.md) side-gun and
  [Liberator](Liberator.md) cluster-bomb, AP shotgun shell,
  [Lasher](Lasher.md) BFR damage, and
  [Mosquito](Mosquito.md) [MAX](MAX.md) damage fixes.
- [3.0.25](3.md.0.25) - [BFR](BattleFrame_Robotics.md)
  acquisition fully operational! [Galaxy](Galaxy.md) gun COF
  and rate of fire decreased.
- [3.0.26](3.md.0.26) - Halloween decor added. Heavy vehicles
  disallowed from purchase inside caverns (_not banished_).
  [BFRs](BattleFrame_Robotics.md) are banished from caverns,
  can no longer decon on walkways, nor can load enemy equipment in
  favorites. "Sanc is full" and crash to desktop fixes.

### November

- [3.0.27](3.md.0.27) - Halloween decor removed and
  [Exploration Merit](Exploration.md) live! Round 1 of BFR
  nerfs. Cavern resecures count towards [BFR
  Imprint](BFR_Imprint.md) every 20 minutes.
- [3.0.29](3.md.0.29) - [BFR](BattleFrame_Robotics.md)
  Cert split into basic [BFR](</BFR_(Certification)>), [BFR
  AI](BFR_Anti-Infantry.md), and [BFR
  AA](BFR_Anti-Aircraft.md) certs. Round 2 of BFR nerfs.
- [3.0.29.1](3.md.0.29.1) - [TR Gunner
  Mortar](Colossus.md) cert fix. Round 3 of BFR nerfs.
- [3.1.2](3.md.1.2) - Global warp queuing, shared vehicle
  experience, cert pre-req dropping, and ground vehicle bailing go
  live! Bailing malfunction added to heavily damaged aircraft. Support
  tools permitted during weapons lock. /ignore includes
  globals/cont-alls. 20 boomer limit implemented.
- [3.1.5](3.md.1.5) - Win condition for ["Run for the
  Guns!"](Run_For_the_Guns.md) [Event](Event.md)
  implemented. Mouse wheel mapping fix.

### December

- [3.1.6](3.md.1.6) - Warpgate queue zone crash fixes
- [3.2.6](3.md.2.6) - Holiday decor added. December Combat
  Event live. Standard [BFRs](BattleFrame_Robotics.md) retired
  with gunner variants becoming standard. All gunner variant BFRs
  armor and speed matched to [Colossus](Colossus.md) model.
  Round 4 of BFR nerfs.
- [3.2.7](3.md.2.7) - [BFR](BattleFrame_Robotics.md)
  Gunner vertical axis fix

## 2005

### January

- [3.2.8](3.md.2.8) - December Combat Event conditions
  removed. Buggy movement and [Fury](Fury.md) rocket tweaks
- [3.2.12](3.md.2.12) - [Battle Rank](Battle_Rank.md)
  23 goes live! [ATV](ATV.md) and
  [Harasser](Harasser.md) now only 1 cert point.
  [BFR](</BFR_(Certification)>) upped to 4,
  [Uni-MAX](</Uni-MAX_(Certification)>) upped to 6, and [Air
  Cavalry](Air_Cavalry.md) upped to 6 cert points.
  [Switchblade](Switchblade.md) buffs.
- [3.3.7](3.md.3.7) - [ANT](ANT.md) tweaks, including
  deploying from inside. Round 5 of
  [BFR](BattleFrame_Robotics.md) nerfs. Multitude of bug
  fixes.
- [3.3.8](3.md.3.8) - [Fury](Fury.md) weapon tweaks.
  Client crash and merit fixes. [Transportation Citation
  Merit](Transportation_Citation.md) added.

### February

- [3.3.9](3.md.3.9) - Initial jolt added to aircraft
  afterburner. Client crash, merit, and keyboard/keymapping fixes.
- [3.3.10](3.md.3.10) - [Aphelion](Aphelion.md) Plasma
  Mortar burn framerate tweak
- [3.4.5](3.md.4.5) - New Global Lattice with [Empire
  Benefits](Empire_Benefits.md) go live! [Broadcast
  warpgate](Broadcast_warpgate.md) mechanics revision, /who
  cr, and [Air Cavalry](Air_Cavalry.md) certification split,
  and new [Lasher](Lasher.md) orb effect implemented.
  Multitude of bug fixes.
- [3.4.6](3.md.4.6) - [Ancient
  Tech](Ancient_Technology.md) acquisition fix. Corpses no
  longer block weapons.
- [3.4.7](3.md.4.7) - Warp Queue broadcast gate,
  [Quasar](Quasar.md) [MAX](MAX.md), and [Command
  Rank](Command_Rank.md) armor display fix.
- [3.4.8](3.md.4.8) - Round 6 of
  [BFR](BattleFrame_Robotics.md) nerfs, including water
  timers. Keymap, merit, respawn, and client crash fixes.
- [3.4.9](3.md.4.9) - "No respawn point" fix

### March

- [3.4.14](3.md.4.14) - Round 7 of
  [BFR](BattleFrame_Robotics.md) nerfs. First revision of
  [Merit Commendations](Merit_Commendations.md). Weapon splash
  on water implemented.

### April

- [3.5.4](3.md.5.4) - [Squad Home
  Base](Squad_Home_Base.md) goes live! Low resolution sounds
  made available. Multitude of fixes.
- [3.5.7](3.md.5.7) - Merit and [Squad Home
  Base](Squad_Home_Base.md), and TR [MAX](MAX.md)
  fixes.
- [3.5.9](3.md.5.9) - Crash to desktop after extensive play
  and vehicles+force domes framerate fixes.

### May

- [3.6.5](3.md.6.5) - Squad Experience Revision and Squad
  Leader chat (sl.md) go live! Major
  [CEP](Command_Experience_Points.md) calculation adjustments.
  All [Medical Terminals](Medical_Terminal.md) now repair
  armor. Multitude of fixes.
- [3.6.6](3.md.6.6) - Squad EXP Radar glow toggleable.
  [BFR](BattleFrame_Robotics.md) and Tank favorites hotfix.
  [Laze](Laze_Pointer.md) fix. Crash to desktop hotfix.
- [3.6.8](3.md.6.8) - Two-year [Term of
  Service](Term_of_Service.md) Merit and [Heavy
  Sniper](Heavy_Sniper.md) Rifle live! Avatar,
  [CE](Combat_Engineering.md),
  [Monolith](Monolith.md), Ground Vehicle, and reticle fixes.
  Squad experience revisions.
- [3.6.8.4](3.md.6.8.4) Heavy Sniper Rifle renamed to [Heavy
  Scout Rifle](Heavy_Scout_Rifle.md),
  [HSR](Heavy_Scout_Rifle.md) Cone of Fire changes, reduced
  [Wall Turret](Phalanx.md) explosion, modified properties of
  aircraft launching

### June

- [3.6.9](3.md.6.9) - [Aircav](Air_Cavalry.md) armor
  adjustments, "Flaklet" added, [Reaver](Reaver.md) damage
  against infantry nerfed, [MAX](MAX.md) adjustments.
- [3.7.3](3.md.7.3) - Support Experience implemented - Support
  Merits tracking - New Player Mission System -
  [Vulture](Vulture.md) - New Population Indicators - Infantry
  Weapon Times.

### July

- [3.7.10](3.md.7.10) - [Support Merit
  Commendations](Support_Merit_Commendations.md) live,
  second-tier Training Missions available
- [3.7.10 Addendum](3.md.7.10_Addendum) - [AMS
  Support](AMS_Support.md) and [Lodestar
  Support](Lodestar_Support.md)
  [Merits](Merit_Commendations.md) changed.

### August

- [3.8.9](3.md.8.9) - [Battle Rank](Battle_Rank.md) 25
  Extension - In-Game Advertisements -
  [Certification](Certification.md) Reallocation
- [3.8.14](3.md.8.14) - Bug Fix Patch

### October

- [3.9.5](3.md.9.5) - [Rabbit](Rabbit.md) Gameplay and
  [LFS](LFS.md) revisions go live! [TR](TR.md)
  [MAX](MAX.md) allowed to shoot while (un)anchoring, minor
  [Vehicle](Vehicle.md)/[BFR](BFR.md) tweaks,
  Help-Star/[HUD](HUD.md) fixes, the end of looting knives.
- [3.9.5 Brief Update](3.md.9.5_Brief_Update) -
  [Rabbit](Rabbit.md) Gameplay adjustments
- [3.9.6](3.md.9.6) [Combat
  Engineering](Combat_Engineering.md) Window added, as well as
  [Rabbit](Rabbit.md) Gameplay adjustments.
- [3.9.7](3.md.9.7) - 2 bug fixes and the annual Halloween
  Jack-O-Lantern '[backpacks](backpack.md)'!

### November

- [3.9.8](3.md.9.8) - Removed Jack-O-Lantern backpacks.
- [3.9.9](3.md.9.9) - Reveal Enemies/Friendlies Ability now
  displays Vehicle Icons - Multiple Bug fixes

### December

- [3.10.0](3.md.10.0) - Added [Tactical
  Overlay](Tactical_Overlay.md),
  [Certification](Certification.md) timer reduced from 24 to 6
  hours, [Buggy](</Assault_Buggy_(Certification)>) and
  [ATV](ATV.md) improvements, holiday decorations.
- [3.10.2](3.md.10.2) - [Enhanced
  Targeting](Enhanced_Targeting.md) can now see BFR shield
  health; multiple changes to Flight [BFRs](BFR.md).

## 2006

### January

- [3.10.3](3.md.10.3) - Upgraded nVIDIA CG DLLs for improved
  graphics performance, removed holiday features, and fixed a bug with
  the [Aphelion](Aphelion.md) and
  [Eclipse](Eclipse.md) lasers.

### March

- [3.11](3.md.11) - Large number of UI and help/training
  system changes.
- [3.11.1](3.md.11.1) - Launch of new /appeal system.
- [3.11.2](3.md.11.2) - Several minor bug fixes.
- [3.11.3](3.md.11.3) - Added option to remove red screen
  flash when taking damage as well as fixing the zoom speed in [third
  person](third_person.md).

### April

- [3.11.4](3.md.11.4) - Balance adjustments for the [Martial
  Law](Martial_Law.md) event on
  [Desolation](Desolation.md).
- [3.11.5](3.md.11.5) - [Merit
  Commendation](Merit_Commendation.md) bug fix.
- [3.11.6](3.md.11.6) - Major Anti Vehicular damage increases
  for the [Phoenix](Phoenix.md), [Lancer](Lancer.md),
  [Striker](Striker.md) and all
  [Anti-Vehicular](Anti-Vehicular.md) [MAX](MAX.md)
  Units.

[\*](category:Patches.md)

### May

- [3.11.11](3.md.11.11) - Updates concerning the Event for
  [PlanetSide](PlanetSide.md)'s Third Anniversary and a fix
  for the Login Problem.

### June

- [3.11.18](3.md.11.18) - Emergency build to address player
  drops / login issues.
- [3.11.20](3.md.11.20) - [Wasp](Wasp.md) Interceptor
  goes live, damage value reversal for the [Rocklet
  Rifle](Rocklet_Rifle.md) against [MAX](MAX.md)
  Units.

### July

- [3.11.24](3.md.11.24) - [Wasp](Wasp.md) acquisition
  bug fixed, hack detection added and changes to the
  [Pounder](Pounder.md) [MAX](MAX.md).
- [3.11.28](3.md.11.28) - Setting changes for the Special
  Event "[Blast Into Battle, Double
  Time](Blast_Into_Battle.md,_Double_Time)".

### December

- [3.11.47](3.md.11.47) - New [Merit
  Commendations](Merit_Commendations.md), additional levels
  for most Merits, and added Christmas decorations.

## 2007

### March

- [3.12.04](3.md.12.04) - New [Combat
  Engineering](Combat_Engineering.md) deployables
- [April Fool's
  Patch](Unnumbered_Patches.md#Game_Update_.2803.2F30.2F2007.29) -
  Undocumented addition of Cake, Pie, and Doughnut Box graphics to
  replace [NC](NC.md), [TR](TR.md), and
  [VS](VS.md) [backpacks](backpack.md) respectively.

### April

- [3.12.12](3.md.12.12) - Revamped
  [Sunderer](Sunderer.md)

### May

- [Anniversary
  Update](</Unnumbered_Patches#Anniversary_2007_Update_(5/18/2007)>) -
  Added [Dragon](Dragon.md), [Scorpion](Scorpion.md),
  [Stinger](Stinger.md), [Eraser](Eraser.md), and
  [Spear](Spear.md)
- [3.12.20](3.md.12.20) - [Combat
  Engineering](Combat_Engineering.md) changes and small
  [Sunderer](Sunderer.md) variant change

### June

- [3.12.23](3.md.12.23) - [One-Manned Field
  Turret](One-Manned_Field_Turret.md) bug fixed, as well as a
  few other bugs

### July

- [3.12.26](3.md.12.26) - 4 Year Anniversary weapons balance
  pass
- [3.12.28](3.md.12.28) - Storyline for the upcoming
  [Phantasm](Phantasm.md) and [Galaxy
  Gunship](Galaxy_Gunship.md), respective
  [certifications](certification.md) have been added, [Home
  Continents](Home_Continent.md) have changed.

### August

- [3.12.29](3.md.12.29) - [Phantasm](Phantasm.md) and
  [Galaxy Gunship](Galaxy_Gunship.md) added, as well as
  several long time bug fixes.
- [3.12.31](3.md.12.31) - Removed
  [Phantasm](Phantasm.md) gun, multiple buffs to [Galaxy
  Gunship](Galaxy_Gunship.md), and increased repair speeds for
  [Combat Engineers](Combat_Engineering.md) and [Advanced
  Engineers](Advanced_Engineering.md).

### November

- [3.13.5](3.md.13.5) - Halloween updates
- [3.13.7](3.md.13.7) - Bugfixes

### December

- [3.13.9](3.md.13.9) - Holiday updates and adjustments to the
  Buggies and [Galaxy Gunship](Galaxy_Gunship.md).

## 2008

### January

- [3.13.10](3.md.13.10) - Holiday Event settings removed,
  minor change to the [Light Scout](Light_Scout.md)
  [certification](certification.md), change to mouse mappings.

### February

- [3.13.11](3.md.13.11) -Valentine's day patch changing [VR
  training](VR_training.md) characters' names, awarding a
  special merit, and adding [Pain Module](Pain_Module.md)
  damage to [cavern](cavern.md) spawn areas.

<!-- -->

- [3.15.2](3.md.15.2) -Added [Expert
  Hacking](Expert_Hacking.md) and [Data
  Corruption](Data_Corruption.md)
  [certifications](certifications.md) and abilities, [Sensor
  Disruptor](Sensor_Disruptor.md), [T-REK](T-REK.md),
  and new [Wasp](Wasp.md) lock-on warning.

### March

- [3.15.4](3.md.15.4) -Corrected a bug with the [Data
  Corruption](Data_Corruption.md)
  [certification](certification.md),
  [viruses](virus.md) in deployables can randomly wear off
  sooner, and minor tweaks to the [Sensor
  Disruptor](Sensor_Disruptor.md).

### May

- [3.15.6](3.md.15.6) -[Markov](Markov.md) and
  [Emerald](Emerald.md) merged into
  [Gemini](Gemini.md). Five-year merit added with free
  [Reinforced Exo-Suit](Reinforced_Exo-Suit.md)
  [certification](certification.md) as the reward.

### August

- [3.15.18](3.md.15.18) -Added overdrive ability to
  [TR](TR.md) [MAXes](MAX.md),
  [Burster](Burster.md) magazine size buff, and changed the
  way [incentives](incentives.md) are calculated.

### December

- [3.15.29](3.md.15.29) -Weapons balance pass. Major overhaul
  of [Lasher](Lasher.md), buffed [Pulsar](Pulsar.md),
  [Punisher](Punisher.md), [Sweeper](Sweeper.md), and
  [Heavy Scout Rifle](Heavy_Scout_Rifle.md).

## 2009

### January

- [3.15.34](3.md.15.34) -Fixed a bug where the
  [Jackhammer](Jackhammer.md) did no damage to
  [MAXes](MAX.md) when firing standard (white ammo) in
  triple-shot mode.
- [3.15.36](3.md.15.36) -[Lasher](Lasher.md) tweaks:
  Fixed hit detection issues, decreased
  [AP](Armor_Piercing.md) damage, and increased lash damage
  and radius.

### February

- [3.15.44](3.md.15.44) -Valentine's day patch changing [VR
  Training](VR_Training.md) characters' names, and awarding a
  [special merit](Ball_.md&_Chain).
- [3.15.45](3.md.15.45) -Removed Valentine's day event
- [3.15.46](3.md.15.46) -Every character received a free
  [certification](certification.md) reset

### March

- [3.15.52](3.md.15.52) -Coder Madness! Increased
  [armor](Vehicle_Armor.md) of
  [Lightning](Lightning.md) and [Medium Battle
  Tanks](Medium_Battle_Tank.md) by 75%. Increased secondary
  knife damage by 200%. Increased [BEP](BEP.md),
  [CEP](CEP.md), and
  [SEP](Support_Experience_Points.md) awarded.
- [3.15.53](3.md.15.53) -Coder Madness part 2. Removed changes
  from previous patch, but buffed [Mosquito](Mosquito.md),
  [Reaver](Reaver.md), [Wasp](Wasp.md),
  [Deliverer](Deliverer.md) and variants, and
  [ATVs](ATV.md). [Lasher](Lasher.md) clip reduced
  to 35.
- [3.15.54](3.md.15.54) -Ends Coder Madness, restoring
  everything to normal. Free [certification](certification.md)
  reset.

### June

- [6th Anniversary Update](6th_Anniversary_Update.md) -Added
  6th year [Term of Service](Term_of_Service.md) merit.
  Increased maximum [Battle Rank](Battle_Rank.md) from 25
  to 40. Free [certification](certification.md) reset.

### August

- [Server Merge
  patch](</Unnumbered_Patches#Server_Merge_and_Lasher_nerf_(08/25/2009)>)
  -Merged [Gemini](Gemini.md) and [Werner](Werner.md),
  [Lasher](Lasher.md) nerf.

### October

- [Unnumbered Patch](Unnumbered_Patches.md) (10/15/09) -Fixed
  bug with five year [Term of Service](Term_of_Service.md)
  awards and dramatically increased [BEP](BEP.md) and
  [CEP](CEP.md) awarded for facility captures.
- [Unnumbered Patch](Unnumbered_Patches.md) (10/22/09) -
  Halloween Event, The Spooktacular Bonus Experience Days

### November

- [Unnumbered Patch](Unnumbered_Patches.md) (11/03/09) -
  Changes [Spooktacular Days](Spooktacular_Days.md)
  [event](event.md) settings back to normal

### December

- [Unnumbered Patch](Unnumbered_Patches.md) (12/09/09) - Game
  client now supports Windows Vista and Windows 7 by default
